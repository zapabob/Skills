...
short_description: Advanced Git worktree management agent that orchestrates parallel development environments with integrated quality assurance and automated w
---
...
```bash
# Feature development
the worktree-manager skill "Create feature worktree for user authentication"

# Bug fix isolation
the worktree-manager skill "Create isolated worktree for critical bug fix"

# Experimental development
the worktree-manager skill "Create experimental worktree for new architecture"
```

### QA Integration
```bash
# Automatic QA on creation
the worktree-manager skill "Create worktree with QA validation"

# Manual QA trigger
the worktree-manager skill "Run QA analysis on auth-feature worktree"

# Continuous monitoring
the worktree-manager skill "Enable background QA monitoring for all worktrees"
```

### Lifecycle Management
```bash
# Status monitoring
the worktree-manager skill "Show status of all worktrees"

# Safe merging
the worktree-manager skill "Merge auth-feature to main (QA validated)"

# Cleanup operations
the worktree-manager skill "Cleanup inactive worktrees older than 7 days"
```
...
the worktree-manager skill "Setup pre-merge QA hooks"
...
- name: Worktree QA Validation
  run: |
    the worktree-manager skill "Validate all active worktrees"
    the worktree-manager skill "Generate merge readiness report"
```
...
```bash
# Restart QA service
the worktree-manager skill "Restart QA service"

# Check QA service status
the worktree-manager skill "Show QA service status"

# Manual QA trigger
the worktree-manager skill "Run QA on specific-worktree"
```
...
the worktree-manager skill "Launch terminal for worktree-name"
...
the worktree-manager skill "Validate worktree-name for merge"
...
# Retry merge
the worktree-manager skill "Merge worktree-name"
```
...
the worktree-manager skill "Setup team worktrees for sprint"

# Developers work in parallel
the worktree-manager skill "Create my-feature worktree"
# ... development work ...
the worktree-manager skill "Mark my-feature ready for review"

# QA team validates
the worktree-manager skill "Run QA on all ready worktrees"

# Merge validated worktrees
the worktree-manager skill "Merge all qa-passed worktrees"
```
...
- name: Setup Parallel Development
  run: the worktree-manager skill "Create CI worktrees"

- name: Parallel QA Validation
  run: the worktree-manager skill "Validate all worktrees"

- name: Merge Ready Worktrees
  run: the worktree-manager skill "Auto-merge qa-passed worktrees"

- name: Cleanup
  run: the worktree-manager skill "Cleanup completed worktrees"
```

### Automated Maintenance
```yaml
# Daily maintenance cron job
0 2 * * * the worktree-manager skill "Daily maintenance: cleanup old worktrees, update QA baselines"
```
...
$ the skill-install skill https://github.com/zapabob/codex-worktree-manager-skill`
**Version**: 2.10.0
