...
function printHelp() {
  const sources = Object.keys(SOURCE_ROOTS).join(",");
  console.log(`agent-skill-unifier

Commands:
  discover
  import-local [--sources ${sources}]
  list
...
Options:
  ...
  --skip-existing      Leave existing target skill directories untouched
  --skip-incompatible  Continue when a skill has no variant for the target
`);
}
...
async function runInstall(positional, flags) {
  ...
  for (const skill of skills) {
    try {
      ...
      if (result.skipped) {
        console.log(`${skill.slug} skipped (${result.reason}): ${result.destinationPath}`);
        continue;
      }

      console.log(`${skill.slug} -> ${result.destinationPath}`);
    } catch (error) {
      if (flags["skip-incompatible"] && error.message.startsWith("No compatible variant")) {
        console.log(`${skill.slug} skipped (incompatible with ${target})`);
        continue;
      }
